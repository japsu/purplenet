...
def remove_network_from_org_page(request, network_id, org_id):
    ...
    if request.method == "POST":
        ...
        return post_confirmation_page(request,
            question="Remove access to %s from %s?" % (
            #question="Remove access to %s from %s? Note that all existing certificates from the organization to the network will be revoked." % ( 
                network.name,
                org.name
            ),
            choices=[
                ("Remove", reverse("remove_network_from_org_page", kwargs=dict(
